...
(function ($) {
    ...
    // Track button clicks
    $(document).on('click', 'a[href*="services-partners.html"]', function() {
        if (typeof trackButtonClick === 'function') {
            trackButtonClick('partners_portal_link', 'navigation');
        }
    });

    $(document).on('click', '.btn[href*="contact.html"]', function() {
        if (typeof trackButtonClick === 'function') {
            trackButtonClick('contact_button', 'cta');
        }
    });

    $(document).on('click', '.btn[href*="services-purchase.html"]', function() {
        if (typeof trackButtonClick === 'function') {
            trackButtonClick('services_button', 'cta');
        }
    });

    // Track service inquiry forms
    $(document).on('submit', 'form[action*="contact-form.php"]', function() {
        var serviceType = 'general_inquiry';
        var pageTitle = document.title.toLowerCase();
        
        if (pageTitle.includes('technical')) serviceType = 'technical_support';
        else if (pageTitle.includes('maintenance')) serviceType = 'maintenance_repair';
        else if (pageTitle.includes('disposal')) serviceType = 'disposal_services';
        else if (pageTitle.includes('service-detail')) serviceType = 'service_detail';
        ...
        if (!error && actionURL != '' && actionURL != undefined) {
            ...
            $.ajax({
                ...
                success: function (result) {
                    ...
                    // Newsletter 전용 성공/실패 메시지 처리
                    if (_this.hasClass('newsletter-submit-btn')) {
                        formObj.find('.newsletter-status').addClass('d-none');
                        ...
                        if (parsedResult.alert && parsedResult.alert.includes('success')) {
                            formObj.find('.newsletter-success').removeClass('d-none');
                            
                            // Track successful newsletter subscription
                            var email = formObj.find('input[name="email"]').val();
                            if (typeof trackNewsletterSubscription === 'function') {
                                trackNewsletterSubscription(email);
                            }
                            
                            // 2초 후 성공 메시지 숨김
                            setTimeout(function() {
                                formObj.find('.newsletter-success').addClass('d-none');
                            }, 2000);
                        } else {
                            formObj.find('.newsletter-error').removeClass('d-none');
                            // 2초 후 실패 메시지 숨김
                            setTimeout(function() {
                                formObj.find('.newsletter-error').addClass('d-none');
                            }, 2000);
                        }
                        return;
                    }
                    ...
                    if (formObj.hasClass('contact-form-style-01') || formObj.hasClass('contact-form-style-03')) {
                        var formType = formObj.hasClass('contact-form-style-01') ? 'general_contact' : 'main_contact';
                    ...
                    if (redirectVal != '' && redirectVal != undefined) {
                        ...
                        // Display notification with custom messages
                        resultsObj.removeClass('alert-success alert-danger alert-warning').hide();
                        
                        // Customize messages based on success/failure
                        var displayMessage = parsedResult.message;
                        if (parsedResult.alert && parsedResult.alert.includes('success')) {
                            displayMessage = '<i class="bi bi-check-circle-fill"></i> Your message has been sent successfully.';
                            resultsObj.addClass('alert-success');
                        } else {
                            displayMessage = '<i class="bi bi-exclamation-triangle-fill"></i> There was a problem sending your message. Please try again later.';
                            resultsObj.addClass('alert-danger');
                        }
                        
                        resultsObj.html(displayMessage);
                        resultsObj.removeClass('d-none').fadeIn('slow').delay(5000).fadeOut('slow');
                    }
                },
                error: function (xhr, status, error) {
                    
                    // Newsletter 전용 에러 메시지 처리
                    if (_this.hasClass('newsletter-submit-btn')) {
                        formObj.find('.newsletter-status').addClass('d-none');
                        formObj.find('.newsletter-error').removeClass('d-none');
                        // 2초 후 에러 메시지 숨김
                    ...
                    resultsObj.removeClass('alert-success alert-danger alert-warning').hide();
                    resultsObj.addClass('alert-danger');
                    resultsObj.html('<i class="bi bi-exclamation-triangle-fill"></i> There was a problem sending your message. Please try again later.');
                    resultsObj.removeClass('d-none').fadeIn('slow').delay(5000).fadeOut('slow');
                }
            });
        }
